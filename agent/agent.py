...
async def entrypoint(ctx: JobContext):
    ...
    print(f"[MeetFlow STT] provider=LiveKit Inference (google/gemini-3.5-transcribe-live)")

    # Connect to the room and auto-subscribe to participant audio
    await ctx.connect(auto_subscribe=AutoSubscribe.AUDIO_ONLY)
    print(f"[MeetFlow STT] Joined room {ctx.room.name} as {ctx.room.local_participant.identity}")
        ...
        async def read_transcripts():
            try:
                async for ev in stt_stream:
                    ...
                    if is_final:
                        print(f"[MeetFlow STT] final transcript received speaker={speaker_name} textLength={len(text)} text=\"{text[:50]}\"")
            except asyncio.CancelledError:
                pass
            except Exception as err:
    ...
    def on_track_subscribed(track: rtc.Track, publication: rtc.RemoteTrackPublication, participant: rtc.RemoteParticipant):
        if participant.identity == "meetflow-stt":
            return
    ...
    def on_participant_disconnected(participant: rtc.RemoteParticipant):
        ...
        humans = [p for p in ctx.room.remote_participants.values() if p.identity != "meetflow-stt"]
        if len(humans) == 0:
